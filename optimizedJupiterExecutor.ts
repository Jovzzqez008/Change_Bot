// optimizedJupiterExecutor.ts - Ejecuta swaps en Jupiter con validaciones numéricas
...
export class OptimizedJupiterExecutor {
  ...
  async buyToken(
    ...
    try {
      ...
      console.log(
        `🪐 Jupiter BUY: ${safeSolAmount} SOL → ${tokensReceived} tokens`,
      );
  ...
  async sellToken(
    ...
    try {
      ...
      console.log(
        `🪐 Jupiter SELL: ${safeTokenAmount} tokens → ${solReceived.toFixed(
          4,
        )} SOL`,
      );
  ...
  async simulateBuy(
    ...
    try {
      const lamports = solToLamports(
        solAmount,
        'buy amount (simulateBuy Jupiter)',
      );
      const quote = await this.getQuote(SOL_MINT, mint, lamports, slippage);
      if (!quote) {
        return {
      ...
      console.log(
        `📄 [PAPER] Jupiter BUY: ${solAmount} SOL → ${rawOut} tokens`,
      );
  ...
  async simulateSell(
    ...
    try {
      ...
      console.log(
        `📄 [PAPER] Jupiter SELL: ${tokenAmount} tokens → ${solReceived} SOL`,
      );
  ...
  // 🔁 Ejecutar el swap (común a buy/sell)
  private async executeSwap(
    ...
    console.log(`🪐 Jupiter ${side.toUpperCase()} executed: ${signature}`);
  ...
  private async getQuote(
    ...
    // 🔄 API nueva: Legacy Swap v1 GET /swap/v1/quote
  ...
  private async getTokenDecimals(mint: PublicKey): Promise<number> {
    ...
    const decimals = (parsed as any)?.parsed?.info?.decimals;
  ...
  private async parseJson<T>(response: Response): Promise<T | null> {
    ...
    try {
      ...
      throw new Error(
        `Failed to parse response: ${error?.message ?? String(error)}`,
      );
    }
  }
}
